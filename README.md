...
- **多策略支持**：TSP（旅行商问题）算法 + 聚类分析策略 + **隔夜住宿混合模式**
...
- **Web 界面**：Streamlit 应用，上传 Excel 即可规划
...
| 行程类型 | 路线 | 适用场景 |
|----------|------|----------|
| 单日往返 🚗 | 公司 → 点位 → 公司 | 距离公司<80 公里的点位 |
| 隔夜住宿 🏨 | Day1: 公司→点位→酒店<br>Day2: 酒店→点位→公司 | 距离公司>80 公里的远途点位 |

系统自动根据距离阈值判断行程类型，优化住宿点选择。
...
YAML 格式配置文件（参考 `presets/overnight_sampling/config.yaml`）
...
**智能混合模式**：自动判断点位距离，选择单日往返或隔夜住宿

**流程**：
1. **点位分类**：根据 `overnight_threshold_km` 阈值，将点位分为
   - 单日点位：距离 < 阈值
   - 隔夜点位：距离 > 阈值
2. **单日规划**：使用 TSP 策略规划单日往返路线
3. **隔夜规划**：
   - 聚类分组隔夜点位
   - 选择住宿点（靠近集群中心）
   - Day 1: 公司 → 点位 → 酒店
