...
**问题**: 步骤 2 验证通过后，`validated_df` 被错误覆盖

**症状**: 
- 步骤 2 地址验证成功
- 步骤 4 生成规划后，坐标仍是近似值 (106.9837, 29.8295)

**原因**:
```python
# 第 648 行 - 错误代码
st.session_state.validated_df = df.copy()  # df = uploaded_df (没有坐标!)
```

**修复**:
- 删除第 648 行的错误赋值
- `validated_df` 已在第 588 行正确设置（带坐标）

**影响**:
- 之前：步骤 2 验证 → 坐标被覆盖 → 步骤 4 使用近似坐标
- 现在：步骤 2 验证 → 坐标保存 → 步骤 4 使用真实坐标
